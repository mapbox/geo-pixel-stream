...
function PixelWriter(gdalDst, bandId, options) {
  ...
  this.on('pipe', function(readable) {
    try {
      assert.deepEqual(_this.metadata.blockSize, readable.metadata.blockSize);
...
PixelWriter.prototype._write = function(data, enc, callback) {
  try {
    assert.deepEqual(this.metadata.blockSize, data.blockSize);
  }
  catch (err) {
    return callback(new Error('Block dimensions do not match'));
  }

  try {
    this._band.pixels.writeBlock(data.offset.x, data.offset.y, data.buffer);
    this._dst.flush();
  }
  catch (err) {
    return callback(err);
